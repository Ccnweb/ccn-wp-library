...
function enqueue_elements_regex($type, $dir, $regex_pattern, $options = array()) {
    ...
    if ($type != 'style' && $type != 'script') return log\error('WORDPRESS_ENQUEUE_FAILED', 'Impossible to enqueue element of unknown type "'.$type.'"');
    ...
    foreach ($element_to_be_enqueued as $el) {
        if ($type == 'style') wp_enqueue_style($options['prefix'].'-'.$i, $options['plugin_url'] . $el['value'], $options['dependencies'], '20190107', 'all');
        else wp_enqueue_script($options['prefix'].'-'.$i, $options['plugin_url'] . $el['value'], $options['dependencies'], '20190107', 'all');
        $i++;
    }
